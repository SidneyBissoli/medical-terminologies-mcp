...
### Changed
- **A mecânica do vocabulário da CID-10 sobe para `@sbissoli/mcp-search` 0.5.0.**
  `src/clients/cid10-vocabulary.ts` fica só com a tabela medida e os nomes de
  sempre; frases, stopwords, singular, nota e ponta inversa vêm de
  `createVocabulary` (locale `pt-BR`, fonte "a CID-10"). Cinco servidores
  carregavam a mesma receita em cópia — regra da Fase 0. Sem mudança de
  comportamento nem de superfície: os 27 testes do vocabulário e o contrato de
  saída passam iguais.
...
- **Telemetria: sessão e cliente (blobs 9 e 10).** O servidor passa a emitir
  ...
  linha do aperto de mão. PRIVACY.md atualizado.
